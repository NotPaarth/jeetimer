...
const SUBJECT_COLORS = {
  ...
  botany: "#22c55e", // green-500
  zoology: "#ef4444", // red-500
...
export default function InsightsView({
  ...
  // Filter logs based on the selected time frame (using study day boundaries)
  const getFilteredLogs = () => {
    const now = new Date()
    const todayStudyStart = getStudyDayStart(now)

    return timeLogs.filter((log) => {
      const logDate = new Date(log.timestamp || log.startTime)

      if (timeFrame === "today") {
        const todayStudyEnd = getStudyDayEnd(now)
        return logDate >= todayStudyStart && logDate <= todayStudyEnd
      } else if (timeFrame === "week") {
        const weekStart = new Date(todayStudyStart)
        weekStart.setDate(todayStudyStart.getDate() - todayStudyStart.getDay())
        return logDate >= weekStart
      } else if (timeFrame === "month") {
        const monthStart = new Date(todayStudyStart.getFullYear(), todayStudyStart.getMonth(), 1)
  ...
  const getTotalTimeBySubject = () => {
    ...
    filteredLogs.forEach((log) => {
      if (totals.hasOwnProperty(log.subject)) {
  ...
  // Prepare data for bar chart (daily distribution using study day boundaries)
  const getBarChartData = () => {
    ...
    filteredLogs.forEach((log) => {
      const logDate = new Date(log.timestamp || log.startTime)
      const studyDayStart = getStudyDayStart(logDate)
      const dayIndex = studyDayStart.getDay() // Use study day start for day calculation
      const minutes = Math.round(log.duration / 60)

      if (dayData[dayIndex] && dayData[dayIndex].hasOwnProperty(log.subject)) {
        dayData[dayIndex][log.subject as keyof (typeof dayData)[0]] += minutes
      }
    })

    // If we're only looking at today, just return today's data
    if (timeFrame === "today") {
      const todayStudyStart = getStudyDayStart(new Date())
      const today = todayStudyStart.getDay()
  ...
  const getTaskStats = () => {
    const stats: Record<string, { total: number; completed: number }> = {
      ...
      botany: { total: 0, completed: 0 },
      zoology: { total: 0, completed: 0 },
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-3xl font-bold capitalize">
                          {Object.entries(totalTimeBySubject).reduce(
                            (max, [subject, time]) => (time > totalTimeBySubject[max] ? subject : max),
                            "physics",
                          )}
                        </p>
                      </div>
                          ...
                          <PieChart>
                            ...
                            >
                              {pieChartData.map((entry, index) => (
                                <Cell
                                  key={`cell-${index}`}
                                  fill={SUBJECT_COLORS[entry.name.toLowerCase() as keyof typeof SUBJECT_COLORS]}
                                />
                              ))}
                            </Pie>
