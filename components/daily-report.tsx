...
export default function DailyReport({ timeLogs, tasks, questionGoal }: DailyReportProps) {
  const [currentDay, setCurrentDay] = useState(() => getStudyDayStart(new Date()))
  const [isExporting, setIsExporting] = useState(false)
  const [showBrowserWarning, setShowBrowserWarning] = useState(false)
  const printRef = useRef<HTMLDivElement>(null)

  // Calculate day end date using study day logic
  const currentDayEnd = getStudyDayEnd(currentDay)

  // Navigate to previous day
  const goToPreviousDay = () => {
    const prevDay = new Date(currentDay)
    prevDay.setDate(prevDay.getDate() - 1)
    setCurrentDay(getStudyDayStart(prevDay))
  }
  ...
  const goToNextDay = () => {
    ...
    const nextStudyDayStart = getStudyDayStart(nextDay)

    // Don't allow navigating to future study days
    if (nextStudyDayStart <= getStudyDayStart(new Date())) {
      setCurrentDay(nextStudyDayStart)
    }
  }

  // Filter logs for current day (using study day boundaries)
  const currentDayLogs = useMemo(() => {
    return timeLogs.filter((log) => {
      const logDate = new Date(log.startTime)
      return logDate >= currentDay && logDate <= currentDayEnd
    })
  }, [timeLogs, currentDay, currentDayEnd])

  // Filter tasks for current day (using study day boundaries)
  const currentDayTasks = useMemo(() => {
    return tasks.filter((task) => {
      const taskDate = new Date(task.createdAt)
      return taskDate >= currentDay && taskDate <= currentDayEnd
    })
