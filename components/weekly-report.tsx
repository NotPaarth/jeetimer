...
export default function WeeklyReport({ timeLogs, tasks, questionGoal }: WeeklyReportProps) {
  ...
  const dailyStudyData = useMemo(() => {
    ...
    const data = days.map((day, index) => {
      ...
      const studyDayStart = getStudyDayStart(dayDate)
      const studyDayEnd = getStudyDayEnd(dayDate)
      ...
      // Fill in actual data using study day boundaries
      currentWeekLogs.forEach((log) => {
        const logDate = new Date(log.startTime)
        if (logDate >= studyDayStart && logDate <= studyDayEnd) {
